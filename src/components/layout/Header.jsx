    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex justify-between items-center h-16">
        ...
        <Link to="/" className="flex items-center space-x-2">
          ...
          <span className="text-xl font-bold text-gray-900 dark:text-white">Emerging Investigations</span>
        </Link>
        ...
        <nav className="hidden md:flex items-center space-x-8">
          <Link 
            ...
            className={`text-gray-700 hover:text-primary-600 dark:text-gray-200 dark:hover:text-primary-400 transition-colors ${
              location.pathname==='/' ? 'text-primary-600 dark:text-primary-400 font-semibold' : ''
            }`}
          >
            Home
          </Link>
          ...
          <div className="relative group">
            <button 
              className="flex items-center space-x-1 text-gray-700 hover:text-primary-600 dark:text-gray-200 dark:hover:text-primary-400 transition-colors"
            ...
            <AnimatePresence>
              {isServicesOpen && (
                <motion.div 
                  ...
                  exit={{opacity: 0,y: -10}}
                  className="absolute top-full left-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 py-2"
                  onMouseEnter={()=> setIsServicesOpen(true)}
                  onMouseLeave={()=> setIsServicesOpen(false)}
                >
                  <Link 
                    to="/services" 
                    className="block px-4 py-2 text-gray-700 hover:bg-primary-50 hover:text-primary-600 dark:text-gray-200 dark:hover:bg-gray-700 dark:hover:text-primary-400 font-semibold"
                  >
                    All Services
                  </Link>
                  ...
                  {services.map((service,index)=> (
                    <Link 
                      key={index} 
                      to={service.path} 
                      className="block px-4 py-2 text-gray-700 hover:bg-primary-50 hover:text-primary-600 dark:text-gray-200 dark:hover:bg-gray-700 dark:hover:text-primary-400 text-sm"
                    >
                      {service.name}
                    </Link>
                  ))}
          ...
          <Link 
            to="/about" 
            className={`text-gray-700 hover:text-primary-600 dark:text-gray-200 dark:hover:text-primary-400 transition-colors ${
              location.pathname==='/about' ? 'text-primary-600 dark:text-primary-400 font-semibold' : ''
            }`}
          >
            About
          </Link>
        ...
        {/* Call Button & Dark Mode Toggle (Mobile) */}
        ...
        <div className="md:hidden flex items-center space-x-2">
          ...
          >
            <SafeIcon icon={isMenuOpen ? FiX : FiMenu} className="w-6 h-6 text-gray-700 dark:text-gray-200" />
          </button>
        </div>
      </div>
    </div>
        ...
        >
          <div className="px-4 py-2 space-y-2">
            <Link 
              to="/" 
              onClick={closeMenu} 
              className="block py-2 text-gray-700 hover:text-primary-600 dark:text-gray-200 dark:hover:text-primary-400 transition-colors"
            >
              Home
            </Link>
            <Link 
